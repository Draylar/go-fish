    ...
    private static void registerFishHandler() {
        LootTableEvents.MODIFY.register((resourceManager, lootManager, identifier, fabricLootSupplierBuilder, lootTableSetter) -> {
            if (identifier.equals(FISH_LOOT_TABLE)) {
                ...
                // Builder<?>)ItemEntry.builder(Items.ROTTEN_FLESH).weight(10)).with((LootPoolEntry.Builder<?>)((LeafEntry.Builder)ItemEntry.builder(Blocks.BAMBOO).conditionally(NEEDS_JUNGLE_BIOME.or(NEEDS_SPARSE_JUNGLE_BIOME).or(NEEDS_BAMBOO_JUNGLE_BIOME))).weight(10))));
